...
MIN_WITHDRAW = 1000
WITHDRAW_FEE = 15.00
INVITE_LOCK_COUNT = 15  # New requirement after fee
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if args and args[0].isdigit() and int(args[0]) != uid:
        ...
        try:
            ...
            # New logic: increase the "post-fee" invite counter if they are in that stage
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if text == "🧠 Logic":
        ...
        cur.execute("SELECT balance, referrals_after_fee FROM users WHERE user_id=?", (uid,))
        bal, refs = cur.fetchone()
        ...
        for i, r in enumerate(rows, 1): lb += f"{i}. {r[0]} — ₱{r[1]:.2f}\n"
...
async def on_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    elif data.startswith("app_"): # ADMIN APPROVAL
        _, target_id, amount = data.split("_")
        # Reset their invite counter to 0 upon fee approval
